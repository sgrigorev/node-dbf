...
(function() {
  ...
  Parser = (function(_super) {
    ...
    function Parser(filename, encoding) {
      ...
      this.base_filename = this.filename.split('.dbf')[0];
    }

    Parser.prototype.parse = function() {
      this.emit('start', this);
      this.header = new Header(this.filename, this.encoding);
      fs.exists("" + this.base_filename + ".fpt", (function(_this) {
        return function(exists) {
          if (exists) {
            _this.header_fpt = new HeaderFPT(_this.base_filename, _this.encoding);
            _this.header_fpt.parse(function(err) {});
            _this.emit('header_fpt', _this.header_fpt);
            return _this.fpt_buffer = fs.readFileSync("" + _this.base_filename + ".fpt");
          }
        };
      })(this));
    ...
    Parser.prototype.parseField = function(field, buffer) {
      ...
      switch (field.type) {
        case 'M':
          if (!this.header_fpt) {
            throw new Error("Memo field was specified but no related .FPT file was found for " + this.filename + ".");
          }
